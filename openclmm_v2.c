...
void MM_dev(float *mA, float *mB, float *results, int Ah, int Aw, int Bw, double *gpuAllocTime, double *gpuCopyTime, double *gpuExecuteTime, int deviceNum){
    ...
    for (i = 0; i < platformCount; i++) {
        ...
        for (j = deviceNum; j < deviceCount-offset; j++) {
            ...
            // Create the context
            cl_context context;
            context = clCreateContext(NULL, 1, &devices[j], NULL, NULL, &errcode_ret);
            ...
            command_queue = clCreateCommandQueue(context, devices[j], 0, &errcode_ret);
            if(errcode_ret != CL_SUCCESS) {
...
int main(int argc, char *argv[]){
    ...
    MM_dev(A,B,results,Ah,Aw,Bw,&gpuAllocateTime,&gpuCopyTime,&gpuExecuteTime, devID);
